...
### 5. Модель данных
Используются существующие сущности помещений и контактов. Для дедупликации нужен критерий уникальности (помещение + идентификаторы контакта: телефон, email, telegram_id). Для логики обновления (SR-CORE02-007, SR-CORE02-008) в сущности контакта должно быть поле «дата последнего изменения»; сопоставление по помещению и полям телефон/email/telegram_id определяет один из исходов: полное совпадение → только обновить дату и лог; частичное совпадение при пустых остальных полях → дополнение; частичное совпадение при непустых отличных значениях → конфликт, отказ.
