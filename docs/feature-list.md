...
| **BE-03** | Аудит-лог: INSERT, UPDATE, DELETE, SELECT; связь событий без раскрытия ПДн (Blind Index / entity_id). Запись IP клиента (X-Forwarded-For / X-Real-IP). Фильтры по дате и entity_id (SR-BE03-008..009). Подписи: адрес помещения контакта и ФИО админа через LEFT JOIN (SR-BE03-010). Список изменённых полей при UPDATE (SR-BE03-011). Ссылка на карточку контакта (SR-BE03-012). Бот пишет telegram_user_id (SR-BE03-013). «Аноним» при user_id=NULL (SR-BE03-014). | Критический | VAL-01 | Готово |
