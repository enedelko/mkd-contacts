...
### 2. Описание и цель
Инструмент загрузки реестра помещений и (опционально) контактов собственников из файлов CSV/XLS. Загрузка идёт в денормализованном виде: одна строка файла = одно помещение + опционально один контакт; на одно помещение может приходиться несколько строк (несколько контактов). Строки без контактной информации принимаются — в этом случае создаётся/обновляется только помещение. Цель — наполнение БД помещениями МКД и контактами собственников для навигации и расчёта кворума.
...
* **SR-CORE01-004:** Система должна извлекать из файла поля помещений: кадастровый номер (или идентификатор помещения), площадь, подъезд, этаж, тип помещения, номер помещения; и при наличии — поля контактов (см. перечень ПДн ниже). Кадастровый номер является обязательным для загрузки: строки без кадастрового номера не принимаются. Контактные колонки и данные являются полностью опциональными: если в строке (или во всём файле) отсутствуют контактные поля, импортируется только помещение без создания/изменения контактов.
...
**Денормализованная загрузка:** Одна строка файла соответствует одному помещению и опционально одному контакту. На одно помещение может приходиться несколько строк (разные контакты) или строки только с данными помещений (без контактов). При импорте система формирует/обновляет записи в таблицах **premises** (уникально по помещению) и, при наличии контактных данных, **contacts** (одна запись на строку с контактом, связь с помещением по кадастровому номеру/ид).
...
|-----------|------|----------------|
| Контактные данные | телефон, email, telegram_id | Полностью опциональны; колонки могут отсутствовать в файле. Если в строке заполнено хотя бы одно из трёх — создаётся/обновляется контакт; иначе импортируется только помещение |
...
Итого: телефон, email и telegram_id — полностью опциональны (и как колонки файла, и как значения в строке). Если в строке есть хотя бы одно из трёх — система создаёт/обновляет контакт; если ни одного — принимается только помещение. Поле «Обращение» — полностью опционально.

**Алиасы колонок и значений (русский язык).** Система должна распознавать в заголовках файла русские названия колонок и в ячейках — русские значения (приведённые к каноническому виду при импорте).

*Алиасы заголовков колонок (примеры):*
...
| entrance | подъезд |
| floor | этаж |
| premises_type | тип_помещения, тип помещения |
| premises_number | номер_помещения, номер помещения, кв, квартира |
| phone | телефон, тел |
| email | почта, e-mail |
...
### 5. Модель данных
Не меняется; список ожидаемых колонок — по CORE-01 (поля помещений; контактные колонки — телефон, email, telegram_id, как обращаться к собственнику — полностью опциональны и могут отсутствовать в файле).
