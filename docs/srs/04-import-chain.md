...
* **SR-CORE01-013 (новые контакты):** При совпадении Room ID (помещения) контакт, не найденный по Blind Index среди существующих записей по этому помещению, сохраняется как новый со статусом «Валидирован».
...
**Нормализация номера помещения (room_id) при импорте:**
...
* **SR-CORE01-019 (паттерны):** Поддерживаемые паттерны номера помещения: арабские цифры (123); цифры с литерами слитно или через дефис (5б, 5-Б, А-23); римские цифры (I–XXX).
...
* **SR-CORE01-022 (римские числа):** Римские числа в диапазоне I–XXX должны конвертироваться в арабские цифры (например, IX → 9, XXX → 30).
...
**Поля помещений** (по LOST-01): кадастровый_номер (или id), площадь, подъезд, этаж, тип_помещения, номер_помещения, общая_площадь_МКД (при необходимости). Значение номера помещения при импорте нормализуется по правилам SR-CORE01-018–SR-CORE01-022; сохранённый нормализованный `room_id` используется как ключ для сопоставления данных в БД.
