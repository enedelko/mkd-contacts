...
* **Security:** Конфигурация не должна содержать мастер-ключи шифрования в репозитории; ключ подаётся в контейнер бэкенда исключительно через Docker Secrets или bind mount (см. BE-02). Секрет с мастер-ключом пробрасывается только в контейнер Backend; контейнер Database не должен иметь доступа к этому секрету или к директории с ключами (архитектурная изоляция).
