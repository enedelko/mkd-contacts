...
* **SR-BE01-008:** Конфигурация развёртывания (Docker Compose и шаблон `.env`) должна поддерживать опциональную переменную окружения `TELEGRAM_SOCKS5_PROXY` (URL SOCKS5-прокси) для исходящего доступа к Telegram Bot API из сервисов `bot`, `backend` и `uptime-check`. Если переменная не задана или пуста, исходящие запросы к Telegram выполняются без прокси (поведение по умолчанию).
...
### 6. Интеграции и API
Опционально: исходящие HTTPS-запросы к `api.telegram.org` из указанных сервисов — см. SR-BE01-008 и модуль BOT (SR-BOT-INF-001).
...
| Опциональный SOCKS5 к Telegram (`TELEGRAM_SOCKS5_PROXY`) | [docs/deploy/05-bot-setup.md](../deploy/05-bot-setup.md), [docker-compose.yml](../../docker-compose.yml) |
