...
* **SR-CORE04-007:** Система должна вычислять и возвращать в ответе API кворума долю площади помещений, по которым есть хотя бы один контакт (со статусом pending или validated) с подтверждённой собственностью в Электронном доме (`contacts.registered_in_ed = 'owner'`): сумма площадей таких помещений / общая площадь. Цель — показатель «% в Электронном доме (с подтверждённой собственностью)» по дому в целом (аналогично расчёту кворума, без порога).
