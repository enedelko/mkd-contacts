...
* **SR-BE02-008:** Для реализации поиска по зашифрованным полям (телефон, email) система должна использовать механизм Blind Index на базе алгоритма HMAC-SHA256. Секретный ключ (pepper) для HMAC должен храниться в переменных окружения бэкенда. Результат вычисления Blind Index сохраняется в индексированные колонки `phone_idx` и `email_idx` в таблице контактов.
...
### 5. Модель данных
**Update:** В БД хранятся зашифрованные строки (или ciphertext + iv по выбранной схеме). Структура полей должна позволять однозначно расшифровать при наличии ключа. Для поиска без расшифровки используются индексированные колонки `phone_idx` и `email_idx` (Blind Index по SR-BE02-008).
...
* **SR-ADM01-008 (Bootstrap):** Первый администратор (Super Admin) должен быть добавлен вручную владельцем системы через прямое выполнение SQL-команды INSERT в контейнере базы данных (например: `docker compose exec db psql -U <user> -d <db> -c "INSERT INTO admins (telegram_id, role) VALUES (...);"`). Это гарантирует отсутствие чувствительных ID в репозитории и конфигурации.
