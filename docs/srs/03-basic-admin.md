...
* **admins** (белый список админов): telegram_id, role (administrator | super_administrator), created_at, full_name (ФИО для отображения в разделе 9 Политики конфиденциальности), premises (помещение, например «Ап.96»). Независимая сущность для авторизации. Использование: проверка при входе (ADM-01), управление составом (ADM-04), отображение перечня контактов администраторов на странице Политики (раздел 9). В миграции при добавлении колонок full_name и premises существующие строки заполняются прочерками. Хранение — только в таблице admins в БД (ADM-01, ADM-04).
