...
* **SR-LOST01-002:** Система должна содержать миграции, создающие таблицу admins (telegram_id, role, created_at; дополнительно full_name, premises — см. модель данных ниже). Поля full_name и premises добавляются отдельной миграцией и при добавлении заполняются прочерками для существующих записей.
...
* **SR-LOST01-004:** Система должна содержать миграции, создающие таблицу собственников/контактов (связь с помещением, контактные данные, registered_in_ed, статус и др. — см. модель данных ниже).
* **SR-LOST01-005:** Система должна содержать миграции, создающие таблицу голосование_в_ОСС (связь с контактом, barrier_vote — позиция по шлагбаумам, vote_format — формат голосования, проголосовал).
...
* **admins** (белый список админов): telegram_id, role (administrator | super_administrator), created_at, full_name (ФИО для отображения в разделе 9 Политики конфиденциальности), premises (помещение, например «Ап.96»). По ADM-09 добавляются policy_consent_at (дата/время принятия согласия с Политикой конфиденциальности) и policy_consent_version (версия политики при принятии). Независимая сущность для авторизации. Использование: проверка при входе (ADM-01), управление составом (ADM-04), отображение перечня контактов администраторов на странице Политики (раздел 9), проверка согласия с Политикой перед доступом к админ-функциям (ADM-09). Хранение — только в таблице admins в БД (ADM-01, ADM-04, ADM-09).

* **premises:** кадастровый_номер, площадь, подъезд, этаж, тип_помещения, номер_помещения (String — для значений «9А», «IX», «10-б»). Иерархия должна позволять каскадный выбор: подъезд → этаж → тип → номер. Поле ФИО (full_name) собственника в схеме помещений и связанных сущностей отсутствует — система не оперирует именами собственников.

* **contacts** (собственники/анкеты): связь с помещением (premise_id), is_owner, phone (ПДн), email (ПДн), telegram_id (ПДн), как_обращаться (ПДн), registered_in_ed (статус в Электронном доме: `none` | `account` | `owner` | null), согласия/версия политики, дата_создания, дата_последнего_изменения (для дедупликации и аудита, CORE-02), ip, статус (pending / validated / inactive). ФИО в таблице контактов не хранится.
Все перечисленные ПДн — только в зашифрованном виде (BE-02)

* **голосование_в_ОСС:** связь с контактом (contact_id), barrier_vote (позиция по шлагбаумам: `for` | `against` | `undecided`), vote_format (формат голосования: `electronic` | `paper` | `undecided`), проголосовал (bool). Данные по участию в ОСС и голосовании из анкет и бота.

* **участие_в_голосовании** (`oss_participation`, CORE-05): связь с помещением (`premise_id` → `premises.cadastral_number`), `share_nominal` (номинал доли из файла), `ownership_share` (суммарная доля по группе строк), `participated` (bool), `imported_at`, `import_batch_id`. Независимая от контактов сущность — снимок данных из внешней выгрузки протокола голосования; заполняется импортом Excel/CSV (см. [17-import-voting-participation.md](17-import-voting-participation.md)). Уникальность: `(premise_id, share_nominal)`.

Связи: контакты → помещение; голосование_в_ОСС → контакт; участие_в_голосовании → помещение. Использование и маппинг из внешних источников — см. CORE-01 (импорт реестра), FE-04 (форма анкеты), CORE-05 (импорт участия в голосовании).

### 6. Интеграции и API
Не применимо для LOST-01 (миграции БД). Команды Alembic выполняются в окружении backend.

* **Публичный API для Политики конфиденциальности (раздел 9):** Система должна предоставлять публичный (без авторизации) эндпоинт, возвращающий для страницы Политики конфиденциальности перечень администраторов Приложения в виде пар «ФИО» и «помещение» (full_name, premises). Эндпоинт не должен отдавать telegram_id, login, role и иные персональные или служебные данные.
...
* **SR-BE02-005:** Мастер-ключ должен монтироваться в контейнер бэкенда как read-only файл по пути `/run/secrets/master_key` с использованием механизма Docker Secrets или bind mount. На хост-машине: файл ключа должен находиться вне директории репозитория (рекомендуемый путь — `/etc/opt/mkd-contacts/master.key`); владение файлом — у системного пользователя, под которым запускается Docker; права доступа на хосте — 400.
* **SR-BE02-006:** Категорически запрещено передавать мастер-ключ через переменные окружения (ENV), так как они доступны через инспекцию контейнера и логирование.
...
* **SR-ADM01-009:** Клиент должен предоставлять кнопку «Выйти» в навигации (видимую только авторизованному пользователю), которая удаляет JWT из localStorage и возвращает интерфейс в неавторизованное состояние.
* **SR-ADM01-010:** Если задана непустая переменная окружения `TELEGRAM_SOCKS5_PROXY`, исходящий запрос backend к Telegram Bot API для резервного получения числового id бота (`getMe`, эндпоинт `/api/auth/telegram/bot-id`) должен выполняться через указанный SOCKS5-прокси; если переменная не задана или пуста — через прямое HTTPS-соединение. Остальной HTTP(S)-трафик backend этот прокси не использует.
