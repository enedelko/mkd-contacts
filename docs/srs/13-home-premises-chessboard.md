...
Реализуется на Frontend (React) и Backend (FastAPI). На главной странице (`/`) блок кворума ОСС (CORE-04) размещается вверху; под ним — компонент `EntrancePicker` (UI-01). После выбора подъезда Frontend запрашивает новый эндпоинт `GET /api/premises/chessboard?entrance=...`, который возвращает помещения подъезда, сгруппированные по этажам, с агрегированными флагами наличия контактов и состоянием по ОСС — без раскрытия ПДн. Над шахматкой отображается % площади помещений, готовых голосовать «ЗА» (та же формула, что и кворум). Клик по ячейке ведёт на форму контакта (FE-04) для неавторизованных пользователей или на `/admin/contacts/list` с фильтрами по подъезду и номеру помещения для админов/суперадминов.

### 2. Описание и цель

На главной странице пользователь видит блок кворума ОСС (как сейчас) и показатель % жителей в Электронном доме по дому; затем выбирает подъезд через `EntrancePicker` (UI-01). После выбора подъезда отображается шахматка — помещения сгруппированы по этажам от верхнего к нижнему; каждый **визуальный ряд** ячеек (до 9 помещений в ряд) предваряется **номером этажа** без рамки, в той же типографике, что и заголовок печати (`chessboard-sheet-heading`). Над шахматкой — показатели % площади с голосами «ЗА» и % в Электронном доме по подъезду. В ячейке: **краткое** обозначение типа помещения (мелко; напр. «кв.»), номер помещения (крупно), иконка Telegram при наличии TG id или телефона, иконка письма при наличии только email; цвет фона — по позиции ОСС (см. SR-FE06-011; конкретные оттенки в CSS). При клике на ячейку открывается форма добавления контакта (обычный пользователь) или страница списка контактов `/admin/contacts/list` с предзаполненными фильтрами по подъезду и номеру помещения (админ/суперадмин). Цель — быстрый обзор помещений подъезда, визуальная оценка охвата и переход к форме/админке по клику.
...
* **SR-FE06-011:** **Семантика цвета фона** ячейки определяется позицией по ОСС и статусом в ЭД (как ниже); **конкретные оттенки и оформление задаются в CSS** (классы `state-*` на ячейке, модификатор контейнера гость/авторизованный). Для пользователя **без JWT** применяется **упрощённая палитра**: одинаковый «зелёный» для **vote_for** и **full**; для **registered** — отдельный приглушённый фон вместо светло-жёлтого; остальные состояния — по смыслу как у полной палитры. Для пользователя **с JWT** — **полная дифференциация**: **full** — насыщенный зелёный; **vote_for** — светло-зелёный; **registered** — светло-жёлтый; **ed_account** — светло-оранжевый; **none** — белый. Смысл состояний: **full** — все из «зелёного» пула планируют голосовать «ЗА»; **vote_for** — часть из пула; **registered** — есть подтверждённая собственность в ЭД (`registered_in_ed = 'owner'`), но нет голосов «ЗА» от пула; **ed_account** — аккаунт ЭД без подтверждённой собственности (`registered_in_ed = 'account'`), нет голосов «ЗА» и нет `owner`; **none** — нет информации. Для full и vote_for учитываются только собственники из «зелёного» пула: `registered_in_ed = 'owner'` или `vote_format = 'paper'`. Голоса «ЗА» от собственников вне пула не дают зелёный цвет. Наведение на ячейку — подсветка рамки (реализация в CSS).
...
* **SR-FE06-015:** Ответ эндпоинта должен содержать этажи в порядке убывания (от макс. к мин.), для каждого этажа — список помещений с полями: `premise_id` (cadastral_number), `premises_type`, `premises_number`, `floor`, а также агрегаты по контактам: `contact_state` (none | ed_account | registered | vote_for | full), `has_telegram_or_phone` (bool), `has_email_only` (bool). ПДн не раскрываются — только флаги наличия.
...
* **SR-FE06-018:** При **печати** страницы с шахматкой: не печатать блок кворума, выбор подъезда, статистику по подъезду над шахматкой, легенду цветов и иконки контактов в ячейках; печатать заголовок **«Подтверждение собственности в ЭД»** и монохромную сетку ячеек с обводкой. Состояния **registered**, **vote_for**, **full** (подтверждение собственности в ЭД / пул owner-paper по правилам бэкенда) отображаются **крестом** из двух линий под углом ±30°; состояния **none** и **ed_account** — без креста. Толщины линий и компоновка — по стилям печати во фронтенде. Типографика заголовка печати и номеров этажей в строках — согласована (класс `chessboard-sheet-heading`).
...
Реализация `contact_state` — раскраска по позиции ОСС и статусу ЭД. Для зелёных состояний (full, vote_for) считается пул активных собственников с `registered_in_ed = 'owner'` **или** `vote_format = 'paper'`; по этому пулу — кто имеет `barrier_vote = 'for'`. Проживающие не учитываются. **Оттенки и печать** на экране — в CSS (см. SR-FE06-011, SR-FE06-018); ниже — семантика состояний.
- **full** — все из пула голосуют «ЗА» (`barrier_vote = 'for'`).
- **vote_for** — часть из пула голосует «ЗА», но не все.
- **registered** — нет голосов «ЗА» от пула, но хотя бы один собственник с подтверждённой собственностью в Электронном доме (`registered_in_ed = 'owner'`).
- **ed_account** — нет голосов «ЗА» и нет `owner`, но хотя бы один собственник с аккаунтом ЭД без подтверждённой собственности (`registered_in_ed = 'account'`).
- **none** — нет информации.
