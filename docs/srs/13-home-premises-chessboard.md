...
* **SR-FE06-011:** **Цвет фона ячейки** определяется позицией по ОСС и статусом в ЭД: **full** (все из «зелёного» пула планируют голосовать «ЗА») — ярко-зелёный; **vote_for** (часть из пула планирует голосовать «ЗА») — светло-зелёный; **registered** (хотя бы один собственник с подтверждённой собственностью в ЭД (`registered_in_ed = 'owner'`), но нет голосов «ЗА» от пула) — светло-жёлтый; **ed_account** (хотя бы один собственник с аккаунтом ЭД без подтверждённой собственности (`registered_in_ed = 'account'`), нет голосов «ЗА» и нет `owner`) — светло-оранжевый; **none** (нет информации) — белый фон. Для full и vote_for учитываются только собственники из «зелёного» пула: те, у кого либо подтверждённая собственность в ЭД (`registered_in_ed = 'owner'`), либо формат голосования «на бумаге» (`vote_format = 'paper'`). Внутри пула: full — все голосуют «ЗА», vote_for — часть голосует «ЗА». Голоса «ЗА» от собственников вне пула (например, только аккаунт ЭД без подтверждения) не дают зелёный цвет. Выбранная/активная ячейка — ярко-синяя рамка (опционально).
...
Реализация `contact_state` — раскраска по позиции ОСС и статусу ЭД. Для зелёных состояний (full, vote_for) считается пул активных собственников с `registered_in_ed = 'owner'` **или** `vote_format = 'paper'`; по этому пулу — кто имеет `barrier_vote = 'for'`. Проживающие не учитываются.
- **full** — все из пула голосуют «ЗА» (`barrier_vote = 'for'`) (ярко-зелёный).
- **vote_for** — часть из пула голосует «ЗА», но не все (светло-зелёный).
- **registered** — нет голосов «ЗА» от пула, но хотя бы один собственник с подтверждённой собственностью в Электронном доме (`registered_in_ed = 'owner'`) (светло-жёлтый).
- **ed_account** — нет голосов «ЗА» и нет `owner`, но хотя бы один собственник с аккаунтом ЭД без подтверждённой собственности (`registered_in_ed = 'account'`) (светло-оранжевый).
