...
* **SR-FE06-011:** **Цвет фона ячейки** определяется позицией по ОСС и статусом в ЭД: **full** (все активные собственники планируют голосовать «ЗА») — ярко-зелёный; **vote_for** (хотя бы один собственник планирует голосовать «ЗА», но не все) — светло-зелёный; **registered** (хотя бы один собственник с подтверждённой собственностью в ЭД (`registered_in_ed = 'owner'`), но нет голосов «ЗА») — светло-жёлтый; **ed_account** (хотя бы один собственник с аккаунтом ЭД без подтверждённой собственности (`registered_in_ed = 'account'`), нет голосов «ЗА» и нет `owner`) — светло-оранжевый; **none** (нет информации) — белый фон. Для full и vote_for учитываются только контакты-собственники (без проживающих). Выбранная/активная ячейка — ярко-синяя рамка (опционально).
...
Реализация `contact_state` — раскраска по позиции ОСС и статусу ЭД (full и vote_for — только по собственникам, проживающие не учитываются):
...
- **registered** — нет голосов «ЗА», но хотя бы один собственник с подтверждённой собственностью в Электронном доме (`registered_in_ed = 'owner'`) (светло-жёлтый).
