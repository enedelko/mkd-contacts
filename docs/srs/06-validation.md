...
* **SR-VAL01-006:** Статус «Валидирован» присваивается автоматически всем контактам, которые были успешно загружены или обновлены в процессе импорта из внешнего реестра (доверенный источник, CORE-01).
...
* **SR-CORE02-005:** Система должна применять правило дедупликации по помещению и контактным полям (телефон, email, telegram_id): при наличии существующей записи для данного помещения поведение определяется степенью совпадения данных — обновление только даты и лог (SR-CORE02-007), дополнение пустых полей (SR-CORE02-008) или отказ при конфликте (SR-CORE02-009); во всех случаях обновления ведётся аудит. Перед вычислением Blind Index (HMAC) для сравнения значения нормализуются: **(а) Email** — приведение к нижнему регистру и удаление пробелов; **(б) Телефон** — удаление всех нецифровых символов; если номер начинается с «8», замена на «7»; итоговый формат для хеширования — строго 11 цифр, начиная с «7» (7XXXXXXXXXX). Любая проверка на дубликаты при импорте (CORE-01) или ручном вводе (ADM-03) должна выполняться через сравнение HMAC-хешей в БД (колонки phone_idx, email_idx), без расшифровки самих полей.
...
### 5. Модель данных
Используются существующие сущности помещений и контактов. Для дедупликации сопоставление выполняется по помещению и HMAC-хешам в колонках `phone_idx`, `email_idx` (BE-02); перед вычислением хеша применяется нормализация по SR-CORE02-005 (email: нижний регистр, без пробелов; телефон: 11 цифр с ведущей «7»). Для логики обновления (SR-CORE02-007, SR-CORE02-008) в сущности контакта должно быть поле «дата последнего изменения»; сопоставление определяет один из исходов: полное совпадение → только обновить дату и лог; частичное совпадение при пустых остальных полях → дополнение; частичное совпадение при непустых отличных значениях → конфликт, отказ.
