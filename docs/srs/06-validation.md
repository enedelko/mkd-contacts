...
* **SR-VAL01-008:** Система должна предоставлять UI (страница модерации) для просмотра списка контактов с кнопками смены статуса «Валидировать» / «Неактуальный» без перезагрузки страницы.
