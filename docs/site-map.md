...
**OPS-02 (отказоустойчивость):** при ответе API 503 (или таймауте критичных запросов) SPA отображает полноэкранную заглушку «Сервис временно недоступен. Попробуйте позже.» без отдельного маршрута — подмена контента при флаге недоступности. После восстановления сервиса обновление страницы возвращает обычное приложение.
...
| `/upload` | Загрузка реестра | **Нет** — при прямом заходе без JWT выполняется редирект на `/login` |
...
| **/upload** (для неавторизованного) | В навигации не показывается; при прямом вводе URL выполняется редирект на `/login`. |
