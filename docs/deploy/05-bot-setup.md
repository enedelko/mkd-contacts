...
Если с «обычного» хоста в интернете HTTPS до домена открывается, а в `getWebhookInfo` по-прежнему `Connection timed out`, часто виноват **провайдер/облако**: на границе сети режут или не маршрутизируют входящие с подсетей Telegram (`149.154.160.0/20`, `91.108.4.0/22`). SOCKS для исходящих запросов бота **не заменяет** входящий webhook. Варианты: запрос в поддержку на **whitelist** этих CIDR на ваш 443; другой тариф/VPS без такого фильтра; вынести публичную точку входа (домен + Nginx) на площадку, где доставка от Telegram не блокируется, с проксированием на ваш бот (архитектуру согласовать с безопасностью и 152-ФЗ).
