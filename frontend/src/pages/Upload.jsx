  ...
  return (
    <div className="upload-page">
      ...
      <section className="instruction" aria-label="Инструкция">
        ...
        <p>Ожидаемые колонки (обязательна только <strong>кадастровый номер</strong>; контактные колонки опциональны — если заполнены, создаётся контакт):</p>
        <ul>
          {EXPECTED_COLUMNS.map((c) => (
            <li key={c}><code>{c}</code></li>
          ))}
        </ul>
        <p>Одна строка файла = одно помещение + опционально один контакт. Строки без контактных данных принимаются (создаётся только помещение). Несколько контактов на одно помещение — несколько строк.</p>
      </section>
